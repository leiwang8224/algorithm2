...
// there can be n! permutations given n digits. we can divide the n! permutations into n groups
// where each group has (n-1)! elements. If we further divide each group into multiple groups we get (n-1) groups
// where each group has (n-2)! elements.

//I’m sure somewhere can be simplified so it’d be nice if anyone can let me know. The pattern was that:
//
//        say n = 4, you have {1, 2, 3, 4}
//
//        If you were to list out all the permutations you have 4 groups (n groups)
    //    where each group has n-1 elements
//
...
// and divide that by the 6 we got from the factorial, which would give you the index of the number
// you want. In the array {1, 2, 3, 4}, k/(n-1)! = 13/(4-1)! = 13/3! = 13/6 = 2. The array {1, 2, 3, 4}
...
public class PermutationSequence {
    ...
    /**
     * Given n digits, find kth permutation
     * @param n
     * @param k
     * @return
     */
    private static String getPermutationSequence(int n, int k) {
        ...
        // factorial array indicates how many permutations total given n (lookup table)
        // factorial[] = {1, 1, 2, 6, 24, ... n!}

        // create list of numbers to get indices
        for (int i = 1; i <= n; i ++) {
            numbers.add(i);
        }
        // numbers = {1,2,3,4}
